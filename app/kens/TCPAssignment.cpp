 ...
 */
 ...
 void TCPAssignment::systemCallback(UUID syscallUUID, int pid, const SystemCallParameter &param) {
   ...
   case SOCKET:
     this->syscall_socket(syscallUUID, pid, std::get<int>(param.params[0]),
                          std::get<int>(param.params[1]),std::get<int>(param.params[1]));
     break;
   case CLOSE:
 ...
 void TCPAssignment::syscall_socket(UUID syscallUUID, int pid, int domain, int type, int protocol) {
   ...
   newSocket.state = TCPState :: CLOSED;
 ...
 void TCPAssignment::syscall_bind(UUID syscallUUID, int pid, int sockfd, struct sockaddr *addr, socklen_t addrlen) {
   ...
   for(auto &socket : socketTable){
     if(socket.first == sockfd){
       TCPSocket &newsocket = socket.second;
     }
   }
 ...
 void TCPAssignment::syscall_connect(UUID syscallUUID, int pid, int sockfd, struct sockaddr *addr, socklen_t addrlen) {
   ...
   TimerPayload timerPayload { clientSocket.fd, syscallUUID};
 ...
 void TCPAssignment::syscall_listen(UUID syscallUUID, int pid, int sockfd, int backlog) {
   for (auto &socket : socketTable) {
     if (socket.first == sockfd) {
       ...
       serverSocket.state = TCPState::LISTEN;
 ...
 void TCPAssignment::syscall_accept(UUID syscallUUID, int pid, int sockfd, struct sockaddr *addr, socklen_t *addrlen) {
   ...
   for (auto &socket : socketTable) {
     if (socket.first == sockfd && socket.second.state == TCPState ::LISTEN) {
       TCPSocket &serverSocket = socket.second;
       TCPSocket newSocket;
 ...
 void TCPAssignment::syscall_close(UUID syscallUUID, int pid, int sockfd) {
   for (auto &socket : socketTable) {
     if (socket.first == sockfd) {
       if (socket.second.state == TCPState :: CLOSE_WAIT) {
         // Send FIN packet
         Packet finPacket(60);
         ...
         socket.second.state = TCPState :: LAST_ACK;
         // 시스템 호출 결과 반환
         this->returnSystemCall(syscallUUID, 0);
         return;
       } 
       else if(socket.second.state == TCPState :: ESTABLISHED) {
        Packet finPacket(60);
        uint8_t flag = 0x01; // FIN 플래그
        finPacket.writeData(FLAGS, &flag, 1);
        sendPacket("IPv4", std::move(finPacket));
        socket.second.state = TCPState :: FIN_WAIT_1;
        // 시스템 호출 결과 반환
        this->returnSystemCall(syscallUUID, 0);
        return;
       }
       else if(socket.second.state == TCPState :: SYN_SENT){
        socket.second.state = TCPState :: CLOSED;
        socketTable.erase(socket.second.fd);
        this->returnSystemCall(syscallUUID, 0);
       }
       
      //  else if (socket.second.state == TCPState :: LISTEN) {
      //    socket.second.state = TCPState :: CLOSED;
      //    this->returnSystemCall(syscallUUID, 0);
      //    return;
      //  }
 ...
 void TCPAssignment::packetArrived(std::string fromModule, Packet &&packet) {
   ...
   if(serverSocket.state == ESTABLISHED && clientSocket.state == FIN_WAIT_1) { // close case
     ...
     if (fin_flag == 0x01) {
       ...
       TimerPayload timerPayload {clientSocket.fd, 0 };
     ...
     socketTable.erase(serverSocket.fd);
     this->returnSystemCall(syscall_ID, 0);
   }
 // 4way handshke end..
 
   else if(serverSocket.state == TCPState :: LISTEN && clientSocket.state == TCPState :: SYN_SENT){ // 3handshake case
     size_t flag_buffer;
     packet.readData(FLAGS, &flag_buffer, 1);
